...
### Local 3-node signing benchmark
After the three local nodes are running, the Rust benchmark CLI can run keygen and keysign benchmarks directly. It reports success/fail counts plus p50/p95/p99 latency for successful requests.

The benchmark runs requests sequentially. For one room, the runtime supports one active MPC session at a time; additional local requests for the same room are queued. The three parties still send protocol messages to the other two peers during each session, but that fanout is not multi-session concurrency.

Benchmark repeated keygen:

```bash
cargo run -p mpc-cli -- benchmark keygen \
  --address ws://127.0.0.1:8090 \
  --threshold 2 \
  --parties 3 \
  --total 100 \
  --wallet-prefix bench-keygen
```

Benchmark repeated keysign. This expects an existing wallet generated by keygen:

```bash
cargo run -p mpc-cli -- benchmark keysign \
  --address ws://127.0.0.1:8090 \
  --wallet-id <wallet_id> \
  --threshold 2 \
  --total 1000 \

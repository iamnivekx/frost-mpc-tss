...
pub struct RequestResponsesBehaviour {
    ...
    pending_requests: HashMap<
        ProtocolRequestId<OutboundRequestId>,
        (
            Instant,
...
impl NetworkBehaviour for RequestResponsesBehaviour {
    ...
    fn on_swarm_event(&mut self, event: FromSwarm) {
        ...
        // Since FromSwarm is not Clone, we need to handle this differently
        // For now, we'll just forward to the first protocol
        // In practice, the swarm should distribute events to all behaviours
        let mut protocols_iter = self.protocols.values_mut();
        if let Some((behaviour, _)) = protocols_iter.next() {
    ...
    fn on_connection_handler_event(
        ...
        let p_name = event.0;
        if let Some((behaviour, _)) = self.protocols.get_mut(p_name.as_str()) {
    ...
    fn poll(
        ...
        'poll_all: loop {
                ...
                if let Ok(payload) = result {
                    if let Some((protocol, _)) = self.protocols.get_mut(&*protocol_name) {
                        if let Err(_) = protocol.send_response(inner_channel, Ok(payload)) {
                            ...
                            debug!("Failed to send response for {:?} on protocol {:?} due to a timeout or due to the connection to the peer being closed. Dropping response", request_id, protocol_name);
                        } else {
            ...
            for (protocol, (behaviour, inbound_queue)) in &mut self.protocols {
                loop {
                            ...
                            match ev {
                                ...
                                RequestResponseEvent::OutboundFailure {
                                        ...
                                        .remove(&(protocol.clone(), request_id).into())
                                    {
                                        Some((started, pending_response)) => {
                                            if let Some(mut pending_response) = pending_response {
                                                // Convert error to RequestFailure and send it
                                                // error will be consumed here
                                                if pending_response
                                    ...
                                    // error was already consumed above, so we use a generic timeout error
                                    // for the event (the actual error was already sent to the response channel)
                                    let out = Event::RequestFinished {
                                        peer,
                                        protocol: protocol.clone(),
                                        duration: started.elapsed(),
                                        result: Err(RequestFailure::Network(
